...
def hash_file_sha256(file_path) -> Optional[str]:
    """
...
def test_hash_functions():
    ...
    try:
        ...
        # Test SHA256
        sha256_hash = hash_file_sha256(test_file)
...
def hash_file(file_path, algorithm="sha256") -> Optional[str]:
    """
    Alias for get_file_hash - for backward compatibility
    """
    return get_file_hash(file_path, algorithm)


def hash_file_md5(file_path) -> Optional[str]:
    """
...
def compare_files_by_hash(file1_path, file2_path, algorithm="sha256") -> bool:
    """
    ...
    Returns:
        True if files have same hash, False otherwise
    """
    try:
        hash1 = get_file_hash(file1_path, algorithm)
        hash2 = get_file_hash(file2_path, algorithm)
